...
def collect_signatures(outdir, prefix, bam_file, mapq):
...
def detect_somatic_connections(workdir, bam_path, max_bpshift, min_sr, min_mapq):
    ...
    print(f'# Detected BNDs {len(all_clusters)}')
    cluster_writer = open(f'{workdir}/somatic_trans_junction_clusters.tsv', 'w')

    # all_clusters = cluster_bp_list(somatic_junc_bps['chr12'], min_sr)
    ...
    print(f'# Trans links: {link_counter}')
...
def connect_clusters(all_bp_clusters, aln_file, cluster_writer):
    ...
    for idx, cluster in enumerate(all_bp_clusters):
        ...
        try:
            cluster_brpk_cov = aln_file.count(cluster_chrom, cluster_brpk - 1, cluster_brpk + 1)
...
def main():
    ...
    aligner = 'minimap2'
    for sample in ['B']:
        workdir = f'/data/home/jdlin/Prostate/{sample}/somatic_trans/T2T/all_sa'
        ...
        for tissue in ['T', 'B']:
            ...
            collect_signatures(workdir, prefix, f'{bamdir}/{sample}-{tissue}.T2T.{bam_suffix}', 20)

        # print(f'Detect somatic junctions {sample} ...')
        # detect_somatic_connections(workdir, f'{bamdir}/{sample}-T.T2T.{aligner}.haptag.sorted.bam', 10, 3, 20)
